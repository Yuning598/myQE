...
:::{admonition} Lemma
Self-Financing Wealth Recursion
...
:::{admonition} Lemma
SPD to Equivalent Martingale Measure
...
:::{admonition} Lemma
Static Budget Problem in Complete Markets
...
**WTS：** 用 Lagrangian 推出最优消费是 SPD 的反函数。
...
关联前文：[连续时间证券市场模型](Asset Pricing/Theoretical AP/02_Dynamic_Asset_Pricing#4.2.1 Trading strategy and gains) · [Asset Pricing/Theoretical AP/cards/part2/HJB 方程与动态规划](Asset Pricing/Theoretical AP/cards/part2/HJB 方程与动态规划)
...
在这个 motivating example 里，唯一相关的 state variable 就是当前财富 $W_t$；因此 [value function](Asset Pricing/Theoretical AP/cards/part2/价值函数) 只需要写成
...
这就是 [envelope condition](Asset Pricing/Theoretical AP/cards/part2/Envelope condition 与 FOC 的区别)：最优点上，消费一单位的边际效用等于保留一单位财富进入下一期的边际价值；它和 FOC 的控制变量一阶条件不同。
...
先看一般的 [portfolio choice problem in continuous time](Asset Pricing/Theoretical AP/cards/part2/连续时间组合选择问题)。  
...
**WTS：** 从 HJB 的 $\phi$ 项解出最优风险资产 dollar holding。
...
**WTS：** 用猜测 $V(t,W)=\dfrac{A(t)}{1-\gamma}W^{1-\gamma}$ 推出常数风险权重。
...
由 [Itô 公式](Asset Pricing/Theoretical AP/cards/part2/Itô 公式)，
...
对照前文：[5.3 Merton Special Case](#5.3 Merton Special Case)（若连 labor income / ratio state $x$ 都消失，就回到 Merton 的 constant portfolio rule）。
