...
## Supply-Demand Structural Equations
Consider a simple commodity market with demand and supply functions:
- **Demand Equation**: $q_i^d = \alpha_0 + \alpha_1 p_i + u_i$
- **Supply Equation**: $q_i^s = \beta_0 + \beta_1 p_i + v_i$
- **Equilibrium Condition**: $q_i^d = q_i^s$

## Exogeneity and Shock Assumptions
...
**Equilibrium Endogeneity Concept**
...
## Equilibrium Mechanism for Endogeneity
...
**整理得到偏差表达式**:
$$
\text{plim } \hat{\alpha}_1 - \alpha_1 = \frac{Cov(p_i, u_i)}{Var(p_i)}
$$
*注：同理可推导供给方程的偏差为
$$
...
## Moment Decomposition for Weighted Bias
利用简化式结果：
...
**Observable Shifters and IV Intuition**
...
## IV Identification Derivation

首先明确联立方程组的结构：
- **需求方程 (Demand)**: $q_i = \alpha_0 + \alpha_1 p_i + u_i$
...
$$
\begin{aligned}
p_i &= \frac{\beta_0 - \alpha_0}{\alpha_1 - \beta_1} + \frac{\beta_2}{\alpha_1 - \beta_1} x_i + \frac{\zeta_i - u_i}{\alpha_1 - \beta_1} \\
q_i &= \frac{\alpha_1 \beta_0 - \alpha_0 \beta_1}{\alpha_1 - \beta_1} + \frac{\alpha_1 \beta_2}{\alpha_1 - \beta_1} x_i + \frac{\alpha_1 \zeta_i - \beta_1 u_i}{\alpha_1 - \beta_1}
\end{aligned}
$$
**1. 验证相关性 (Relevance)**:
利用简化式 计算 $x_i$ 与内生变量 $p_i$ 的协方差：
$$
\begin{aligned}
...
## IV Estimator Definition
对于需求参数 $\alpha_1$，一个自然的估计量是：
$$
\hat{\alpha}_{1,IV} = \frac{\text{sample covariance between } x_i \text{ and } q_i}{\text{sample covariance between } x_i \text{ and } p_i}
$$

## Analogy-Principle IV Derivation
...
**Haavelmo Macroeconomic Equilibrium**

这是一个经典的宏观计量模型，展示了国民收入核算体系中的同时性偏误。

## Haavelmo Structural Equations
...
## Latent True-Regressor Model
理论上，持久消费 $C_i^*$ 与持久收入 $Y_i^*$ 成正比：
$$
C_i^* = k Y_i^*, \quad 0 < k < 1
$$
...
C_i - c_i &= k(Y_i - y_i) \\
C_i &= k Y_i + (c_i - k y_i) \\
C_i &= k Y_i + u_i, \quad \text{where } u_i \equiv c_i - k y_i
\end{aligned}
$$
...
## Production Structural Equation
考虑横截面样本中厂商的 Cobb-Douglas 生产函数：
$$
Q_i = A_i \cdot L_i^{\phi_1} \cdot \exp(v_i), \quad 0 < \phi_1 < 1
$$
其中：
- $Q_i$: 产出；$L_i$: 劳动投入。
...
解得最优劳动投入：
$$
L_i = \left( \frac{w}{p} \right)^{\frac{1}{\phi_1-1}} (A_i B \phi_1)^{\frac{1}{1-\phi_1}}
$$

## Transmission Bias Derivation
对生产函数 (3.2.11) 取对数：
$$
...
**Regressors, Instruments, and Exogeneity Classes**
...
$$
y_i = \mathbf{z}_i' \boldsymbol{\delta} + \varepsilon_i \quad (i = 1, 2, \dots, n),
$$
...
## Linearity and Parameter Interpretation
...
## Stationarity, Ergodicity, and Instrument Count
1. **平稳性 (Stationarity)**: 
...
## Orthogonality Moment Interpretation
...
## Regressor Classification and Self-Instrumenting
...
## Deep Dive into Regressor Classes
...
$$
E[\mathbf{x}_i \cdot (y_i - \mathbf{z}_i' \boldsymbol{\delta})] = \mathbf{0} \quad (K \times 1)
$$
...
得到待解的线性方程组：
$$
\underset{(K \times L)}{\boldsymbol{\Sigma}_{xz}} \underset{(L \times 1)}{\boldsymbol{\delta}} = \underset{(K \times 1)}{\boldsymbol{\sigma}_{xy}}
$$
...
**GMM Definition and Objective**

**Moment Functions and Sample Moments**
...
&\equiv \mathbf{s}_{xy} - \mathbf{S}_{xz} \tilde{\boldsymbol{\delta}}
\end{aligned}
$$
其中：
...
**Simultaneous-Equation Moments**

由样本矩条件 $\mathbf{g}_n(\tilde{\boldsymbol{\delta}}) = \mathbf{0}$ 导出的联立方程组为：
$$
\mathbf{S}_{xz} \tilde{\boldsymbol{\delta}} = \mathbf{s}_{xy}
$$
...
$$
\hat{\boldsymbol{\delta}}_{IV} = \mathbf{S}_{xz}^{-1} \mathbf{s}_{xy} = \left(\frac{1}{n} \sum_{i=1}^n \mathbf{x}_i \mathbf{z}_i'\right)^{-1} \frac{1}{n} \sum_{i=1}^n \mathbf{x}_i y_i
$$
...
$$
\hat{\boldsymbol{\delta}}(\widehat{\mathbf{W}}) \equiv \arg \min _{\tilde{\boldsymbol{\delta}}} J(\tilde{\boldsymbol{\delta}}, \widehat{\mathbf{W}})
$$
其中目标函数定义为：
$$
...
**Closed-Form GMM and Sampling Error**

## Quadratic GMM Objective
...
$$
J(\tilde{\boldsymbol{\delta}}, \widehat{\mathbf{W}}) = n \cdot (\mathbf{s}_{xy} - \mathbf{S}_{xz} \tilde{\boldsymbol{\delta}})' \widehat{\mathbf{W}} (\mathbf{s}_{xy} - \mathbf{S}_{xz} \tilde{\boldsymbol{\delta}})
$$

## GMM First-Order Condition
...
整理得一阶条件 (FOC)：
$$
\underset{(L \times K)}{\mathbf{S}_{xz}'} \underset{(K \times K)}{\widehat{\mathbf{W}}} \underset{(K \times 1)}{\mathbf{s}_{xy}} = \underset{(L \times K)}{\mathbf{S}_{xz}'} \underset{(K \times K)}{\widehat{\mathbf{W}}} \underset{(K \times L)}{\mathbf{S}_{xz}} \underset{(L \times 1)}{\tilde{\boldsymbol{\delta}}}
$$

## Closed-Form GMM Estimator
根据 GMM 的一阶条件，在 Assumption 3.2 和 3.4 成立的前提下（保证矩阵可逆），$\hat{\boldsymbol{\delta}}(\widehat{\mathbf{W}})$ 的推导过程如下：
...
\hat{\boldsymbol{\delta}}(\widehat{\mathbf{W}}) &= (\mathbf{S}_{xz}' \widehat{\mathbf{W}} \mathbf{S}_{xz})^{-1} \mathbf{S}_{xz}' \widehat{\mathbf{W}} \mathbf{s}_{xy}
\end{aligned}
$$
:::{admonition} Note
Special Case: $K=L$
若 $K=L$，则 $\mathbf{S}_{xz}$ 为可逆方阵。GMM 估计量退化为：
$$
...
整理得**抽样误差表达式**：
$$
\hat{\boldsymbol{\delta}}(\widehat{\mathbf{W}}) - \boldsymbol{\delta} = (\mathbf{S}_{xz}' \widehat{\mathbf{W}} \mathbf{S}_{xz})^{-1} \mathbf{S}_{xz}' \widehat{\mathbf{W}} \bar{\mathbf{g}}
$$
...
$$
\operatorname{Avar}(\hat{\boldsymbol{\delta}}(\widehat{\mathbf{W}})) = (\boldsymbol{\Sigma}_{xz}' \mathbf{W} \boldsymbol{\Sigma}_{xz})^{-1} \boldsymbol{\Sigma}_{xz}' \mathbf{W} \mathbf{S} \mathbf{W} \boldsymbol{\Sigma}_{xz}(\boldsymbol{\Sigma}_{xz}' \mathbf{W} \boldsymbol{\Sigma}_{xz})^{-1}
$$
...
## Delta-Method Derivation for Wald Tests
...
$$
\widehat{\mathbf{S}} \equiv \frac{1}{n} \sum_{i=1}^n \hat{\varepsilon}_i^2 \mathbf{x}_i \mathbf{x}_i'
$$
...
## Efficient GMM Weighting Matrix Summary
...
**Asymptotic Power and Optimality**
...
## Overidentification Test Derivation
...
- 两边同时左乘工具变量 $\mathbf{x}_i$：
$$
\mathbf{x}_i y_i = \mathbf{x}_i \mathbf{z}_i' \boldsymbol{\delta} + \mathbf{x}_i \varepsilon_i
$$
- 对所有样本求均值（即求样本矩）：
$$
\frac{1}{n} \sum_{i=1}^n \mathbf{x}_i y_i = \left( \frac{1}{n} \sum_{i=1}^n \mathbf{x}_i \mathbf{z}_i' \right) \boldsymbol{\delta} + \frac{1}{n} \sum_{i=1}^n \mathbf{x}_i \varepsilon_i
$$
- 代入符号定义，得到**基本分解式**：
$$
...
## Subset Tests of Orthogonality Conditions
...
#### 2SLS as an IV Estimator (将 2SLS 视为特定 IV 估计量)
...
- **代入推导 (Substitution)**:
  将 $\hat{\mathbf{Z}} = \mathbf{P}\mathbf{Z}$ 代入 IV 公式中：
  $$
\begin{aligned}
...
#### When Regressors Are Predetermined (当解释变量均为先定变量)
...
3.  **检验统计量的计算**:
  ...
  - 计算公式：
$$
LR = \frac{SSR_{restricted} - SSR_{unrestricted}}{\hat{\sigma}^2}
$$
  - 该公式展示了 GMM 框架下的似然比原理与经典线性回归中 $F$ 检验（在大样本下）的一致性。

#### Testing a Subset of Orthogonality Conditions: Revisited
...
##### Hausman 检验统计量 (Hausman and Taylor, 1980)
...
:::{admonition} Wald 检验与 Hausman 检验
:class: note
